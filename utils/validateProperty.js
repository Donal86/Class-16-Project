  ...
  if (!hasAllProperties(obj)) {
    ...
    if (!dateValidation(market_date)) {
      process.messages.push(
        `Invalid market date, it must be in past and in mysql format : ${DATE_FORMAT}`
      );
      process.valid = false;
    }
    ...
    if (
      !hasProperties(location, ["address"]) &&
      !hasProperties(location, ["coordinates"])
    ) {
      process.messages.push(
        "you don't have any location address or coordinates!"
      );
      process.valid = false;
    } else if (
      hasProperties(location, ["address"]) &&
      ...
      if (
        !checkIfNumber(location["coordinates"]["lat"]) ||
      ...
      if (!stringValidation(location.address, "address")) {
        if (!hasProperties(location.coordinates, coordinatesProps)) {
          process.messages.push(
            "No address found! and one of coordinates properties is missing!"
          );
          process.valid = false;
        } else {
          if (
            ...
            process.messages.push(
              "Invalid coordinates value, should be number!"
            );
            process.valid = false;
          }
        }
