...
function housesArrayProduce(houses) {
    ...
    houses.forEach((house, i) => {
        ...
        qurArr[i] = [link, myDate, location.country, location.city, location.address, location.coordinates.lat,
            location.coordinates.lng, parcel_m2, gross_m2, net_m2, size.rooms, price.value, price.currency, description, title, strImg, sold];
    });
    return qurArr;
}
...
function getAllCitiesStatus(houses) {
  ...
  function insertIntoDatabase(report, houses, cityStatus) {
    let storeHousesQuery = "REPLACE INTO property (link, market_date, location_country, location_city, location_address, location_coordinates_lat,";
    storeHousesQuery += " location_coordinates_lng, size_parcelm2, size_grossm2, size_netm2, size_rooms, price_value, price_currency, description,";
...
function handleResultsOfPromises(data, res) {
  if (Array.isArray(data) &&  data.length < 1) {
    return res.status(404).json({ message: "The json file is empty!" });
...
function reconizeFileUpload() {
  ...
  module.exports = {
    housesArrayProduce, statusArray, insertIntoDatabase,
    readJsonFile, fetchJsonURL, loopInNormalization,
    loopInValidation, isEmptyObject, handleResultsOfPromises,
    reconizeFileUpload
  };