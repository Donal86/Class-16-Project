...
class HouseMap extends React.Component {
    ...
    render () {
        ...
        const markerHouses = PropertiesStore.map((house, key) => 
            <Placemark
                key={key} 
        ...
        let startingCenter = PropertiesStore.reduce((accu, house) => {
            if(accu['lat'] === undefined || accu['lng'] === undefined) {
                accu['lat'] = 0;
                accu['lng'] = 0;
            } else {
                accu['lat'] = accu['lat'] + house.lat / PropertiesStore.length;
                accu['lng'] = accu['lng'] + house.lng / PropertiesStore.length;
            }
            return accu
        }, {})
